...
%define card "AVM GmbH|Fritz X USB ISDN TA"
...
Source1: dkms-fxusb-use-autoconf-header.patch
Source2: dkms-fxusb-remove-usb_driver_owner.patch
Source3: dkms-fxusb-remove-DECLARE_MUTEX_LOCKED.patch
Source4: dkms-fxusb-update-INIT_WORK-changes.patch
Source5: dkms-fxusb-update-usb_complete_t-functions.patch
...
CLEAN="make clean"
AUTOINSTALL="yes"
PATCH[0]="dkms-fxusb-use-autoconf-header.patch"
PATCH_MATCH[0]="^2\.6\.(19)|([2-9][0-9]+)|([1-9][0-9][0-9]+)"
PATCH[1]="dkms-fxusb-remove-usb_driver_owner.patch"
PATCH_MATCH[1]="^2\.6\.(1[6-9])|([2-9][0-9]+)|([1-9][0-9][0-9]+)"
PATCH[2]="dkms-fxusb-remove-DECLARE_MUTEX_LOCKED.patch"
PATCH_MATCH[2]="^2\.6\.(2[4-9])|([3-9][0-9]+)|([1-9][0-9][0-9]+)"
PATCH[3]="dkms-fxusb-update-INIT_WORK-changes.patch"
PATCH_MATCH[3]="^2\.6\.([2-9][0-9]+)|([1-9][0-9][0-9]+)"
PATCH[4]="dkms-fxusb-update-usb_complete_t-functions.patch"
PATCH_MATCH[4]="^2\.6\.(19)|([2-9][0-9]+)|([1-9][0-9][0-9]+)"
EOF

tar c . | tar x -C $RPM_BUILD_ROOT/usr/src/%module-%version-%release/

for p in %{_sourcedir}/dkms-fxusb-use-autoconf-header.patch \
         %{_sourcedir}/dkms-fxusb-remove-usb_driver_owner.patch \
         %{_sourcedir}/dkms-fxusb-remove-DECLARE_MUTEX_LOCKED.patch \
         %{_sourcedir}/dkms-fxusb-update-INIT_WORK-changes.patch \
         %{_sourcedir}/dkms-fxusb-update-usb_complete_t-functions.patch;
do
